...
def load_run_events(run, run_dir):
    ...
    for fname, cols in EVENT_BINARIES.items():
        ...
        if data is None:
            ...
            continue
        source_ev = np.asarray(data['ev'], dtype='i4')
        idx = _align(ev, source_ev)
        present = idx >= 0
        for name in cols:
            src = np.asarray(data[name])
            # Treat malformed bin files the same as missing columns: leave them
            # empty so we don't use them for cuts.
            if src.ndim != 1 or src.shape[0] != source_ev.shape[0]:
                print('  {}: {}:{} malformed (shape {}), leaving empty'.format(
                    run, fname, name, src.shape))
                continue
